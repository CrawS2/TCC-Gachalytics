...
<div class="character-banner">
    <div class="left-object">
        <?php
            $sql = 'SELECT nome_personagem, foto_personagem, desc_personagem FROM personagem WHERE id_personagem = 24';
            $resultado = $conn->query($sql);
            $linha = $resultado->fetch_object();
        ?>
...
<div class="tab-content" id="aba1" style="display: block;">
    ...
    <div class='skills'>
        <?php
            $sql ="SELECT habilidade.nome_habilidade, habilidade.tipo_habilidade, habilidade.desc_habilidade, habilidade.info_habilidade, skill_type.nome_type, skill_type.id_type FROM habilidade RIGHT JOIN skill_type ON habilidade.tipo_habilidade = skill_type.id_type WHERE habilidade.personagem_habilidade = 24 AND habilidade.tipo_habilidade not in ('5','6')";
            $resultado = $conn->query($sql);
    ...
    <div class="weapon">
        <?php
            $sql = "SELECT nome_arma, foto_arma, desc_arma from arma where id_arma = 24";
            $resultado = $conn->query($sql);
            $linha = $resultado->fetch_object();
        ...
        <div class="weapon-skill">
            <?php
                $sql ="SELECT habilidade.nome_habilidade, habilidade.tipo_habilidade, habilidade.desc_habilidade, habilidade.info_habilidade, skill_type.nome_type, skill_type.id_type FROM habilidade RIGHT JOIN skill_type ON habilidade.tipo_habilidade = skill_type.id_type WHERE habilidade.personagem_habilidade = 24 AND habilidade.tipo_habilidade = 5";
                $resultado = $conn->query($sql);
                while($linha = $resultado->fetch_object())
                {
                    echo"<div class='skill-box'>
