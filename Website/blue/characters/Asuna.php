...
<style>
    ...
    <?php
        $sql ="SELECT estilo_elemento from elemento where id_elemento = 3";
...
<div class="character-banner">
    <div class="left-object">
        <?php
            $sql = 'SELECT nome_personagem, foto_personagem, desc_personagem FROM personagem WHERE id_personagem = 2';
            $resultado = $conn->query($sql);
            $linha = $resultado->fetch_object();
        ?>
    ...
    <div class="character-img">
        <img style="height: 22vh;" src="../../../Gachalytics/imagens/guias_blue/Characters/<?= htmlspecialchars($linha->foto_personagem); ?>" alt="character-image">
    </div>
</div>
...
<div class="tab-content" id="aba1" style="display: block;">
    ...
    <div class='skills'>
        <?php
            $sql ="SELECT habilidade.nome_habilidade, habilidade.tipo_habilidade, habilidade.desc_habilidade, habilidade.info_habilidade, skill_type.nome_type, skill_type.id_type FROM habilidade RIGHT JOIN skill_type ON habilidade.tipo_habilidade = skill_type.id_type WHERE habilidade.personagem_habilidade = 2 AND habilidade.tipo_habilidade not in ('5','6')";
            $resultado = $conn->query($sql);
            ...
            <div class='skill-header'>
                ...
                <div class='skill-info'>
                    <span class='skill-name'>$linha->nome_habilidade</span>
    ...
    <div class="weapon">
        <?php
            $sql = "SELECT nome_arma, foto_arma, desc_arma from arma where id_arma = 2";
            $resultado = $conn->query($sql);
            $linha = $resultado->fetch_object();
            echo "<img class='weapon-img' src='../../../Gachalytics/imagens/guias_blue/Weapons/$linha->foto_arma'>
            <div class='weapon-info'>
                <span class='weapon-name'>$linha->nome_arma</span><br>
                <span class='weapon-desc'>$linha->desc_arma</span>
            </div>";
        ?>
        <div class="weapon-skill">
            <?php
                $sql ="SELECT habilidade.nome_habilidade, habilidade.tipo_habilidade, habilidade.desc_habilidade, habilidade.info_habilidade, skill_type.nome_type, skill_type.id_type FROM habilidade RIGHT JOIN skill_type ON habilidade.tipo_habilidade = skill_type.id_type WHERE habilidade.personagem_habilidade = 2 AND habilidade.tipo_habilidade = 5";
                $resultado = $conn->query($sql);
                while($linha = $resultado->fetch_object())
                {
                    echo"<div class='skill-box'>
                <div class='skill-header'>
                    <div class='skill-type'><span style='font-weight:600; position:relative; top: 35%'>$linha->nome_type</span></div>
                    <div class='skill-info'>
                        <span class='skill-name'>$linha->nome_habilidade</span>W
                        <span class='skill-add'>$linha->info_habilidade</span></div></div>
                    <div class='skill-desc'>$linha->desc_habilidade<span></span></div>
                </div>";
                }
